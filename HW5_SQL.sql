DROP DATABASE IF EXISTS HW5_SQL;
CREATE DATABASE HW5_SQL;
USE HW5_SQL;
...
/* 1. Создайте представление, в которое попадет информация о пользователях 
...
CREATE OR REPLACE VIEW young_users AS
	SELECT 
		...
		p.hometown
	FROM users u 
	JOIN profiles p ON u.id = p.user_id AND p.birthday >= '2003-01-01';
    
SELECT * FROM young_users;
    
/* 2. Найдите кол-во, отправленных сообщений каждым пользователем и 
выведите ранжированный список пользователей, указав имя и фамилию пользователя, 
количество отправленных сообщений и место в рейтинге 
(первое место у пользователя с максимальным количеством сообщений) . 
(используйте DENSE_RANK)*/

SELECT 
	id,
    user_name,
    cnt_msg,
    DENSE_RANK() OVER(ORDER BY cnt_msg DESC) AS rank_msg
FROM
(
	SELECT
		u.id AS id,
		CONCAT(u.firstname, ' ', u.lastname) AS user_name,
		COUNT(m.from_user_id) AS cnt_msg
	FROM users u
	JOIN messages m ON u.id = m.from_user_id
	GROUP BY u.id
) AS msg;

/* 3. Выберите все сообщения, отсортируйте сообщения по возрастанию даты 
отправления (created_at) и найдите разницу дат отправления между соседними 
сообщениями, получившегося списка. (используйте LEAD или LAG) */
SELECT
	id,
    body,
    created_at,
    ROUND((UNIX_TIMESTAMP(created_at) - UNIX_TIMESTAMP(LAG(created_at) OVER w ))/ 60, 0) AS lag_date_sec
FROM messages
