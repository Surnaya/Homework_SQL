DROP DATABASE IF EXISTS HW4_SQL;
CREATE DATABASE HW4_SQL;
USE HW4_SQL;
...
SELECT 
	COUNT(l.id) AS users_under12_likes
FROM likes l
RIGHT JOIN profiles p ON p.birthday >= '2011-01-01' AND p.user_id = l.user_id;

-- 2. Определить кто больше поставил лайков (всего): мужчины или женщины.
SELECT 
	p.gender,
	COUNT(l.id) AS likes_leader
    
FROM likes l
RIGHT JOIN profiles p ON p.user_id = l.user_id
GROUP BY p.gender
ORDER BY likes_leader DESC
LIMIT 1;
...
/* 4. (по желанию)* Пусть задан некоторый пользователь. Из всех друзей 
этого пользователя найдите человека, который больше всех написал ему сообщений. */

SELECT 
	u.id,
    CONCAT(u.firstname, ' ', u.lastname) AS user_name,
    COUNT(m.id) AS msg_count_to_1
FROM users u
LEFT JOIN messages m ON to_user_id = 1 AND u.id = m.from_user_id
GROUP BY u.id
ORDER BY msg_count_to_1 DESC
LIMIT 1;